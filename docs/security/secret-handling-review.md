...
- shared-login bootstrap now defaults to disabled in production and rejects default credentials if enabled
...
- admin console stores operator session material in browser local storage
...
5. disable shared-login bootstrap unless there is a written approval and temporary incident-use justification
...
- verify `SHARED_LOGIN_ENABLED=false` unless exception approved
